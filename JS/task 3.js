...
const loremIpsum = ["Lorem", "ipsum", "dolor", "sit amet", "consectetur", "adipiscing", "elit",
                    "Suspendisse", "aliquet", "turpis", "et", "mauris", "efficitur", "vel", "venenatis",
                    "sem", "sollicitudin", "Mauris", "vulputate", "id", "nisi", "ac",];
let defaultDate = new Date(1971, 0, 1);
let defaultUser = "Иван";
let defaultHashtag = "js";
for(let i = 0; i < 20; i++){
    let string = loremIpsum[_id] + " " + loremIpsum[_id+1] + " " + loremIpsum[_id+2] + `#js${_id}`;
    newTweet = new Tweet(_id, string, defaultDate.setDate(defaultDate.getDate() + _id), defaultUser + `${_id}`);
...
const Module = (function() {
    ...
    const getTweets = function(skip = 0, top = 0, filterConfig){
        ...
        for(let Tweet of arrTweets){
            ...
            if(filterConfig.dateFrom !== undefined){
                 if(!(Tweet.createdAt >= filterConfig.dateFrom && Tweet.createdAt <= filterConfig.dateTo)){
                    continue;
                 }
            }
            if(filterConfig.text !== undefined){
                 if(!(Tweet.text.toLowerCase().includes(filterConfig.text.trim().toLowerCase()))){
                    continue;
                 }
            }
            if(filterConfig.hashtags !== undefined){
                 if(!(Tweet.text.toLowerCase().includes(filterConfig.hashtags.trim().toLowerCase()))){
                    continue;
                 }
            }
            if(skip !== 0){
            ...
            else{  //если необходимое количество твитов пропущено, заполняем массив
                if(top !== 0){
    ...
    const validateTweet = function(Tweet){
        if (Tweet.id === undefined || Tweet.text === undefined || Tweet.createdAt === undefined || Tweet.author === undefined){
            return false;
        }
        else if(Tweet.text.length >= 280 || Tweet.text === '' || Tweet.author === ''){
            return false;
        }
        return true;
    }
    const addTweet = function(string){
        if(this.user !== undefined){  //неавторизованный пользователь не может писать твиты
            let date = new Date();
            newTweet = new Tweet(_id, string.trim(), date, this.user);
            if(validateTweet(newTweet)){  //проверка твита на валидность
                arrTweets.push(newTweet);
                _id++;
                return true;
            }
    ...
    const editTweet = function(id, string){
        ...
        if(this.user === getTweet(id).author && string !== '' && string !== undefined && string.length <= 280){
    ...
    const validateComment = function(Comment){
        if (Comment.id === undefined || Comment.text === undefined || Comment.createdAt === undefined || Comment.author === undefined){
            return false;
        }
        else if(Comment.text.length >= 280 || Comment.text === '' || Comment.author === ''){
            return false;
        }
        return true;
    }
    const addComment = function(id, string){
        if(this.user !== undefined){  //неавторизованный пользователь не может писать комментарии
            let date = new Date();
            newComment = new Comment(_id, string.trim(), date, this.user);
            if(validateComment(newComment)){  //проверка комментария на валидность
